...
function Proper_length_cal_vladimir()
	...
	curveLengthsO = zeros(size(curveFiles,1),1);
	for i=1:size(curveFiles,1)
        ...
        % calculate the length as if no correction would be needed
        % proper coordinates
        curve = corCoordsScaling.*curve;
        % list of segments beginnings and ends
        curveE = curve(2:end,:);
        curve = curve(1:end-1,:);
        % (dx,dy)
        curve = (curve - curveE);
        % |(dx,dy)|
        curve = sqrt(sum(curve.^2,2));
        curveLengthsO(i) = sum(curve);
